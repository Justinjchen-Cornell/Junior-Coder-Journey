...
## 真实场景（它在哪工作）
- 🏠 生活：整理书架"分两堆"、快递分拣按地区
- 💻 互联网：多数语言内置排序的基础（C qsort/JS sort）、Excel 大表、数据库 ORDER BY、MapReduce 分治范式（大数据的第一课）
- 💰 金融：订单簿按价格排序撮合、回测中的大量排序

## 局限性（它做不到什么）
- 最坏 O(n²)：已排序数据 + 首元素 pivot → 实测爆栈
- 不稳定；对小数组不如插入排序（工程版是混合的）
- 递归深度最坏 O(n) → 大数据需随机化 pivot 或改迭代
- 它是"平均最快"不是"一定最快"——要稳定排序请用归并
